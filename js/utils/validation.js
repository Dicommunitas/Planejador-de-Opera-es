 ...
 * @returns {boolean} Verdadeiro se for um número, falso caso contrário 
 */ 
...
export function validateOperation(operation) { 
  ...
  if (!isNumber(operation.volume)) { 
    errors.push('Volume inválido'); 
  } 
 
  if (!isNumber(operation.flowRate) || operation.flowRate <= 0) { 
...
export function validateTankData(tankData) { 
  ...
  // Produto pode ser uma string vazia para tanques vazios 
  if (typeof tankData.produto !== 'string') { 
    errors.push('Produto inválido'); 
  } 
 
  if (!isNumber(tankData.disponivelEnvio)) { 
    errors.push('Volume disponível para envio inválido'); 
  } 
 
  if (!isNumber(tankData.espacoRecebimento)) { 
